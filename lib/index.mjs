...
export function parseNpmOutputToJSON(output) {
  ...
  outputLines.forEach((line) => {
    ...
    if (
      trimmedLine === 'Tarball Contents' ||
      trimmedLine === 'Tarball Details'
    ) {
      currentSection = trimmedLine;
      return;
    }

    if (currentSection === 'Tarball Contents') {
    ...
    } else if (currentSection === 'Tarball Details') {
      ...
      if (matches.length === 2) {
        ...
        // @ts-expect-error
        result.tarballDetails[key] = matches[1];
      }
    }
  });
